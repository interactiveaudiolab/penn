...
class ARDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        """Retrieve the PyTorch DataLoader for training"""
        return ar_loader(self.name, 'train', self.batch_size, self.num_workers, penne.VOICE_ONLY)

    def val_dataloader(self):
        """Retrieve the PyTorch DataLoader for validation"""
        return ar_loader(self.name, 'valid', self.batch_size, self.num_workers, True)

    def test_dataloader(self):
        """Retrieve the PyTorch DataLoader for testing"""
        return ar_loader(self.name, 'test', self.batch_size, self.num_workers, True)

###############################################################################
# Data loader
###############################################################################

def ar_loader(dataset, partition, batch_size=64, num_workers=None, voiceonly=penne.VOICE_ONLY):
    """Retrieve a data loader"""
    if dataset == 'BOTH':
        dataset_obj = torch.utils.data.ConcatDataset([
            ARDataset('MDB', partition, voiceonly),
            ARDataset('PTDB', partition, voiceonly)
        ])
    else:
        dataset_obj = ARDataset(dataset, partition, voiceonly)
    return torch.utils.data.DataLoader(
        dataset=dataset_obj,
        batch_size=batch_size,
        shuffle='test' not in partition,
...
def ar_collate_fn(batch):
    features, targets, ar = zip(*batch)
    col_features = torch.cat(list(features))
    col_targets = torch.cat(list(targets))
    col_ar = torch.cat(list(ar))
    # (batch, window_size), (batch), (batch, ar_size)
    return (col_features, col_targets, col_ar)
